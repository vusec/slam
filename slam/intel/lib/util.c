...
void config()
{
	...
	pr_info("CPU: %s\n", cpu_name()); // TODO assert via lscpu
	pr_info("Kernel: %s\n", kernel_name()); // TODO assert via uname
	pr_info("Gadget: %s\n", gadget_name());
	pr_info("\n");
}
...
void init_rand()
{
	pr_verbose("=====[Randomness initialization]=====\n");
	int seed = time(0);
	srand(seed);
	pr_verbose("Seeding randomness with seed: %d.\n\n", seed);
}
...
{
	for (int i = 0; i < nr; i++) {
		void *hint = (void *)(lrand() & 0x00007ffffffff000UL);
...
/* Find the unique virtual kernel address inside the kernel's direct mapping
 * of physical memory, that points to the same physical address as @va.
 * The use of this function requires root privilege.
 */
...
void visualize_medians(uint64_t latencies[256])
{
	int64_t lat;
	// Compute the "overall" median: the median of all access latencies.
	uint64_t copy[256];
	memcpy(copy, latencies, 256*sizeof(uint64_t));
	uint64_t overall_median = median_sort(copy, 256);

	pr_verbose("overall median: %lu\n    ", overall_median);
	for (int col = 0; col < 16; col++)
		pr_verbose("%4d", col);
	pr_verbose("\n");
	for (int row = 0; row < 16; row++) {
		pr_verbose("%3d ", 16*row);
		for (int col = 0; col < 16; col++) {
			lat = (int64_t)(latencies[row*16+col] - overall_median);
			pr_verbose("%4ld", lat);
		}
		pr_verbose("\n");
	}
